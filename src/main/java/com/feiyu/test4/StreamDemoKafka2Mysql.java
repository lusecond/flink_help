...
//        env.setStateBackend(new FsStateBackend("file:///Users/temp/cp/"));
        ...
        env.addSource(new FlinkKafkaConsumer011<>(topic_ExactlyOnce, new JSONKeyValueDeserializationSchema(true), props))
                .setParallelism(1)
//                .print();
                .timeWindowAll(Time.seconds(10)).apply(
            new AllWindowFunction<ObjectNode, List<ObjectNode>, TimeWindow>() {
                @Override
                public void apply(TimeWindow window, Iterable<ObjectNode> values, Collector<List<ObjectNode>> out) throws Exception {
                    ...
                    if (model.size() > 0) {
                        System.out.println("10 秒内收集到 mysqlExactlyOnce_test 的数据条数是：" + model.size());
                        out.collect(model);
                    }
                }
            }
...
//        .print();
        .addSink(new MySqlTwoPhaseCommitSinkMore()).name("MySqlTwoPhaseCommitSink");
